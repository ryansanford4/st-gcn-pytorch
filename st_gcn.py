...
class Model(nn.Module):
    ...
    def __init__(self, in_channels, num_class, edge_importance_weighting, **kwargs):
        ...
        # load graph
        A = torch.zeros((12, 12), dtype=torch.float32, requires_grad=False)
    ...
    def forward(self, x, graph_args):
        # Get adjacency matrix
        self.graph = Graph(**graph_args)
        A = torch.tensor(self.graph.A, dtype=torch.float32, requires_grad=False).cuda()
        self.register_buffer('A', A)
        # data normalization
        N, C, T, V = x.size()
