...
from non_local_block import nonlocalnet
from run_roi_align import run_roi_align
...
class Bottleneck(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, n_s=0, depth_3d=47, ST_struc=('A','B','C'), non_local=False):
...
class P3D(nn.Module):
    ...
    def __init__(self, block, layers, modality='RGB', config=None,
        shortcut_type='B', num_classes=400, dropout=0.5, ST_struc=('A','B','C')):

        self.non_local = False
        self.roi_align = False
        self.pool_roi_align = None
        if config is not None:
            self.non_local = config.non_local
            self.roi_align = config.roi_align
            self.crop_Size = config.crop_size
            self.pool_roi_align = config.pool_roi_align
        ...
        self.depth_3d=sum(layers[:4])# C3D layers are only (res2,res3,res4),  res5 is C2D
        ...
        self.layer3 = self._make_layer(block, 256, layers[2], shortcut_type, stride=2, non_local=self.non_local)
        self.layer4 = self._make_layer(block, 512, layers[3], shortcut_type, stride=2, non_local=self.non_local)

        # GCN
        if self.pool_roi_align == 'gcn':
            self.gcn_simple = GCN(config, input_channels=512 * block.expansion)

        self.avgpool = nn.AdaptiveAvgPool2d((1, 1))
        self.avgpool3d = nn.AdaptiveAvgPool3d((1, 1, 1))
        self.avgpool3d_gcn = nn.AdaptiveAvgPool3d((None, 1, 1))
        self.adaptive_avgpool1d = nn.AdaptiveAvgPool1d(1)
        self.adaptive_avgpool2d = nn.AdaptiveAvgPool2d((None, 1, 1))
        self.adaptive_maxpool1d = nn.AdaptiveMaxPool1d(1)
        self.adaptive_maxpool2d = nn.AdaptiveMaxPool2d((None, 1))
    ...
    def _make_layer(self, block, planes, blocks, shortcut_type, stride=1, non_local=False):
        ...
        for i in range(1, blocks):
            # Add non-local block to every other residual block
            if non_local and i % 2 == 0:
                layers.append(block(self.inplanes, planes,n_s=self.cnt,depth_3d=self.depth_3d,ST_struc=self.ST_struc, non_local=True))
            else:
    ...
    def forward(self, x, bbox_input):
        batch_size, num_frames, num_bbox, _ = bbox_input.size()
        ...
        if self.roi_align:
            ...
            output = run_roi_align(x, bbox_input, self.crop_Size)
            # output = torch.squeeze(torch.squeeze(self.avgpool3d_gcn(roi_align_output), dim=-1), dim=-1)
        else:
            x = self.avgpool3d(x)
...
def P3D199(pretrained=False,modality='RGB',**kwargs):
    ...
    if pretrained==True:
        if modality=='RGB':
            pretrained_file='/media/nas-570-002-nfssamba/rsanford/activity_recognition/group_activity_recognition/p3d_rgb_199.checkpoint.pth.tar'
        elif modality=='Flow':
            pretrained_file='p3d_flow_199.checkpoint.pth.tar'
        pretrained_state = torch.load(pretrained_file)['state_dict']
