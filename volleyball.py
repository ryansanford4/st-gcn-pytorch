...
from resize_preserve_aspect import ResizeAspectPreserving
...
def load_samples_sequence(anns, tracks, images_path,
                          frames, image_size, num_boxes=12,):
    """
    ...
    for i, (sid, src_fid, fid) in enumerate(frames):
        ...
        if len(boxes[-1]) != num_boxes:
            boxes[-1] = np.vstack([boxes[-1], boxes[-1]
...
class VolleyballDataset(data.Dataset):
    ...
    def load_samples_sequence(self, select_frames):
        ...
        resize_preserve_aspect = ResizeAspectPreserving(self.feature_size)

        images, boxes, bbox_images = [], [], []
        activities, actions = [], []
        for i, (sid, src_fid, fid) in enumerate(select_frames):
            ...
            for j, track in enumerate(self.tracks[(sid, src_fid)][fid]):

                y1, x1, y2, x2 = track
                # w1, h1, w2, h2 = x1 * OW, y1 * OH, x2 * OW, y2 * OH
                
                temp_boxes[j] = np.array([x1 * OW, y1 * OH, x2 * OW, y2 * OH], dtype=int)
            ...
            for bb in boxes[-1]:
                bbox_img = img.crop((bb))
                bbox_img = resize_preserve_aspect(bbox_img)
                bbox_img = transforms.ToTensor()(bbox_img)
                bbox_img = transforms.Normalize(imagenet_mean, imagenet_std)(bbox_img)
                bbox_images.append(bbox_img)
            activities.append(self.anns[sid][src_fid]['group_activity'])

        # images = np.stack(images, axis=0)
        bbox_images = np.stack(bbox_images, axis=0)
        activities = np.array(activities, dtype=np.int32)
        bboxes = np.vstack(boxes).reshape([-1, self.num_boxes, 4])
        actions = np.hstack(actions).reshape([-1, self.num_boxes])

        # convert to pytorch tensor
        bbox_images = torch.from_numpy(bbox_images).float()
        bboxes = torch.from_numpy(bboxes).float()
        # actions = torch.from_numpy(actions).long()
        activities = torch.from_numpy(activities).long()
        return bbox_images, bboxes, activities